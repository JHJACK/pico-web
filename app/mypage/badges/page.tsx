  ...
  return (
    <main style={{ minHeight: "100vh", background: "#0d0d0d", color: "#e8e0d0" }}>
      <nav style={{
        ...
        height: 56, background: "rgba(13,13,13,0.96)", backdropFilter: "blur(20px)",
        borderBottom: "0.5px solid rgba(255,255,255,0.06)",
        display: "flex", alignItems: "center", padding: "0 20px", gap: 16,
      }}>
        <button onClick={() => router.back()} style={{ background: "none", border: "none", color: "#c8bfb0", fontSize: 13, cursor: "pointer" }}>← 뒤로</button>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 16px 80px" }}>
        ...
        <div style={{ padding: "28px 0 24px", textAlign: "center" }}>
          <h1 style={{ fontSize: 20, fontWeight: 600, margin: 0, marginBottom: 6 }}>🎖️ 뱃지 도감</h1>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {ALL_CATEGORIES.map((cat) => {
            ...
            const recent  = isOwned ? badges.find(b => b.category === cat) : null;
            return (
              <button
                key={cat}
              ...
              >
                ...
                {/* 제목 */}
                <div style={{ fontSize: 12, fontWeight: 600, color: isOwned ? meta.color : "#555", marginBottom: 4 }}>
                  {meta.title}
                </div>
                <div style={{ fontSize: 10, color: "#444" }}>{meta.subtitle}</div>

                {/* 획득 횟수 / 미획득 */}
                <div style={{ marginTop: 10 }}>
                  {isOwned ? (
                    <span style={{ fontSize: 11, background: `${meta.color}20`, color: meta.color, borderRadius: 6, padding: "2px 8px", fontWeight: 600 }}>
                      {earned}회 획득
                    </span>
                  ) : (
                    <span style={{ fontSize: 10, color: "#333" }}>미획득</span>
                  )}
                </div>
              </button>
            );
        ...
        {selected && (() => {
          ...
          const earned = countMap[selected] ?? 0;
          const history = badges.filter(b => b.category === selected);
          return (
            <div style={{
              marginTop: 20, background: `linear-gradient(135deg, ${meta.color}10 0%, rgba(13,13,13,0) 100%)`,
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#555", marginTop: 2 }}>{meta.subtitle}</div>
                </div>
              </div>
              <p style={{ fontSize: 13, color: "#a09688", lineHeight: 1.6, marginBottom: 16 }}>{meta.desc}</p>

              {history.length > 0 ? (
                <>
                  <div style={{ fontSize: 12, color: "#555", marginBottom: 8 }}>획득 기록</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {history.map((h, i) => (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#a09688", background: "rgba(255,255,255,0.03)", borderRadius: 8, padding: "6px 10px" }}>
                ...
                <div style={{ fontSize: 12, color: "#333", textAlign: "center", padding: "8px 0" }}>
                  아직 획득하지 못했어요. 도전해보세요!
                </div>
              )}
            </div>
        ...
        {badges.length > 0 && !selected && (
          <div style={{ marginTop: 28 }}>
            <p style={{ fontSize: 12, color: "#555", marginBottom: 12 }}>최근 획득 내역</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {badges.slice(0, 10).map((b, i) => {
                const meta = BADGE_META[b.category];
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, background: "rgba(255,255,255,0.02)", border: "0.5px solid rgba(255,255,255,0.05)", borderRadius: 12, padding: "10px 14px" }}>
                    <span style={{ fontSize: 20 }}>{meta?.emoji}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 500, color: meta?.color }}>{meta?.title}</div>
                      <div style={{ fontSize: 11, color: "#444" }}>{weekLabel(b.week_start)}</div>
                    </div>
                  </div>
                );
              })}
