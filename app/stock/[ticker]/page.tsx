...
// ─── 로고 ────────────────────────────────────────────────────────────────────
function TickerLogo({ src, ticker, size = 36 }: { src: string; ticker: string; size?: number }) {
...
export default function StockChartPage() {
  ...
  const tvUrl = `https://s.tradingview.com/widgetembed/?hideideas=1&symbol=${tvSym}&interval=${TV_PERIOD[period]}&hidesidetoolbar=1&hidetoptoolbar=1&theme=dark&style=3&locale=kr&backgroundColor=%23141414`;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d0d", color: "#e8e0d0", paddingBottom: 48 }}>
      <style>{`
        @keyframes shimmer {
          0%   { background-position: 200% 0; }
          100% { background-position: -200% 0; }
        }
        .period-btn { transition: background 0.15s, color 0.15s, border-color 0.15s; }
        .order-tab  { transition: background 0.15s, color 0.15s; }
        .quick-btn  { transition: background 0.12s; }
        .quick-btn:active { background: rgba(250,202,62,0.25) !important; }
      `}</style>
      ...
      <div style={{ padding: "0 14px", display: "flex", flexDirection: "column", gap: 12, marginTop: 14 }}>

        {/* ── 가격 히어로 ──────────────────────────────────────────────────── */}
        <div style={{ padding: "4px 4px 0" }}>
          {loading ? (
            <><Skeleton w={200} h={38} radius={8} /><div style={{ height: 8 }} /><Skeleton w={130} h={16} radius={6} /></>
          ) : (
            <>
              <div style={{ ...NUM_MONO, fontSize: 40, fontWeight: 300, color: "#e8e0d0", lineHeight: 1.1 }}>
                {kr ? data?.formattedPrice : data?.formattedKRW ?? "—"}
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 8 }}>
                <span style={{
                  ...NUM_MONO, fontSize: 14, color: accentColor,
                  background: accentBg,
                ...
                {!kr && (
                  <span style={{ ...NUM_MONO, fontSize: 13, color: "#5c5448" }}>
                    {data?.formattedPrice ?? ""}
                  </span>
                )}
        ...
        <Card>
          ...
          <div style={{ display: "flex", borderBottom: "0.5px solid rgba(255,255,255,0.06)" }}>
            {([["buy", "매수"], ["sell", "매도"]] as [OrderTab, string][]).map(([tab, label]) => (
              <button key={tab} onClick={() => { setOrderTab(tab); clearAmount(); }}
                className="order-tab"
                style={{
          ...
          <div style={{ padding: "16px 16px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {quickAmounts.map((n) => (
                <button key={n} onClick={() => addAmount(n)} className="quick-btn"
                  style={{
              ...
              <button onClick={setAll} className="quick-btn"
                style={{
                  flex: 1, padding: "9px 0", fontSize: 12, fontWeight: 500,
                  borderRadius: 10,
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {[
              ...
              <div key={label} style={{
                ...
                {loading
                  ? <Skeleton w="80%" h={13} />
                  : <div style={{ ...NUM_MONO, fontSize: 13, color: colored ? accentColor : "#e8e0d0" }}>
                      {value}
                    </div>
                }
              </div>
